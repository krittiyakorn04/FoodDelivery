...
CREATE TABLE `Store` (
    ...
    `accountStatus` ENUM('PENDING', 'ACTIVE', 'SUSPENDED') NOT NULL DEFAULT 'PENDING',
...
CREATE TABLE `OrderRound` (
    ...
    `status` ENUM('OPEN', 'CLOSED', 'DELIVERED') NOT NULL DEFAULT 'OPEN',
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
...
CREATE TABLE `Payment` (
    ...
    `method` ENUM('PROMPTPAY', 'BANK_TRANSFER', 'CASH') NOT NULL,
...
-- AddForeignKey
ALTER TABLE `OrderRound` ADD CONSTRAINT `OrderRound_storeId_fkey` FOREIGN KEY (`storeId`) REFERENCES `Store`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `DeliveryRound` ADD CONSTRAINT `DeliveryRound_storeId_fkey` FOREIGN KEY (`storeId`) REFERENCES `Store`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `DeliveryRound` ADD CONSTRAINT `DeliveryRound_orderRoundId_fkey` FOREIGN KEY (`orderRoundId`) REFERENCES `OrderRound`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `Order` ADD CONSTRAINT `Order_deliveryRoundId_fkey` FOREIGN KEY (`deliveryRoundId`) REFERENCES `DeliveryRound`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
